...
function Kaaran(props: Props) {
  return <h2 className="flex flex-col justify-center text-xl">{props.name}</h2>;
}

function KaaranCard(props: CardProps) {
  return (
    <div
      className={`relative bg-zinc-100 dark:bg-slate-800 px-5 py-5 rounded-lg overflow-hidden`}
    >
      <div
...
export default function WhySection() {
  ...
  const [displayText, setDisplayText] = useState(
    siteConfig.reasons[0].description
  );
  ...
  return (
    <section className="lg:flex justify-between items-center gap-x-10 mt-10 lg:mt-0 w-full">
        ...
        {siteConfig.reasons.map((item) => {
          return (
            <div
              ...
              style={{
                fontWeight:
                  item.index === currentSelection ? "bolder" : "normal",
              }}
      ...
      <div className="text-center lg:text-left lg:w-[55%]">
        <h2 className="text-3xl lg:text-5xl font-semibold mb-4 lg:mb-10">
          Why Choose Devansh International Documentation Consultancy (DIDC)?
        </h2>
        <p
          className={`hidden lg:block lg:text-xl lg:transition-all lg:duration-500 lg:transform ${animationState}`}
        >
          {displayText}
        </p>
